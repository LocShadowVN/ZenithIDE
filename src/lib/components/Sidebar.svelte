...
<aside class="w-60 bg-[#1e293b] flex flex-col border-r border-[#0f172a]">
  <div class="h-9 flex items-center justify-between px-4 text-[11px] font-bold uppercase text-slate-400">
    Explorer
  ...
  <div class="h-6 flex items-center px-4 text-[11px] font-bold uppercase text-slate-400">
    Zenith_Workspace
  </div>
  <div class="flex-1 overflow-y-auto py-1">
    {#if nodes.length === 0}
      <div class="text-center text-xs text-slate-600 mt-10 px-4">
        No files opened.<br/>Click the icon above to create one.
      </div>
    {:else}
      {#each nodes as node}
        <button class="w-full text-left pl-5 pr-3 py-[3px] flex items-center gap-1.5 hover:bg-[#334155] text-slate-300 transition-colors" on:click={() => !node.is_dir && openFile(node)}>
          {#if node.is_dir}
            <span class="text-slate-400"><Icon name="folder" size={16} /></span>
          {:else}
            <span class="text-blue-400"><Icon name="file" size={16} /></span>
          {/if}
