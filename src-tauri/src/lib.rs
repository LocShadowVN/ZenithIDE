...
fn init_workspace(app: &tauri::AppHandle) {
    let path = app.path().app_local_data_dir().expect("Failed to get local data dir").join("zenith_workspace");
    let _ = std::fs::create_dir_all(&path);
    
    let c_file = path.join("main.c");
    if !c_file.exists() {
        let _ = std::fs::write(&c_file, "#include <stdio.h>\n\nint main() {\n    printf(\"Hello, World! (C)\\n\");\n    return 0;\n}");
    }
    let cpp_file = path.join("main.cpp");
    if !cpp_file.exists() {
        let _ = std::fs::write(&cpp_file, "#include <iostream>\n\nint main() {\n    std::cout << \"Hello, World! (C++)\" << std::endl;\n    return 0;\n}");
    }
    ...
    if !html_file.exists() {
        let _ = std::fs::write(&html_file, "<!DOCTYPE html>\n<html>\n<head><title>Hello</title></head>\n<body>\n    <h1>Hello, World! (HTML)</h1>\n</body>\n</html>");
    }
}
